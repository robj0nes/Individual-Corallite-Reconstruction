...
from .context_block import TemporalContextBlock
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        # Freeze TCM layer if specified in the config_vit set-up in train.py
        if 'tcm' in self.config.freeze:
            with torch.no_grad():
                self.tcm = TemporalContextBlock(inplanes=1024, window_size=17, repeat_mode=True, reduce=True)
        else:
            self.tcm = TemporalContextBlock(inplanes=1024, window_size=17, repeat_mode=True, reduce=True)

    def forward(self, x):
        keyframe = x[:, int((x.shape[1] - 1) / 2), :, :].unsqueeze(1).repeat(1, 3, 1, 1)
        keyframe_x, keyframe_features = self.hybrid_model(keyframe)

        all_frames_hidden_features = []
        for i in range(x.shape[1]):
            frame_x, frame_features = self.hybrid_model(x[:, i, :, :].unsqueeze(1).repeat(1, 3, 1, 1))
            all_frames_hidden_features.append(frame_x)

        snip = len(all_frames_hidden_features)

        stacked_features = torch.cat(all_frames_hidden_features, dim=0)
        temporal_blended_features = self.tcm(stacked_features, snip)

        x = self.patch_embeddings(temporal_blended_features)  # (B, hidden. n_patches^(1/2), n_patches^(1/2))
        x = x.flatten(2)
        x = x.transpose(-1, -2)  # (B, n_patches, hidden)

        embeddings = x + self.position_embeddings
        embeddings = self.dropout(embeddings)
        return embeddings, keyframe_features
